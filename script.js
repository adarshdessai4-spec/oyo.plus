...
// singular-ui guard — 20251005js2
(function(){
  try {
    // 1) Remove duplicates beyond the first (header/logo, quick-icons, bottom-nav)
    const keepFirst = (sel) => {
    ...
    // 2) Ensure the one bottom-nav sits directly under <body> (not inside random wrappers)
    const nav = document.querySelector('nav.bottom-nav');
    if (nav && nav.parentElement !== document.body) {
      document.body.appendChild(nav);
    }

    // 3) Prevent any future duplicate insertions via insertAdjacentHTML
    ...
    Element.prototype.insertAdjacentHTML = function(position, html) {
      try {
        if (typeof html === 'string' &&
          ...
          return; // A nav already exists — block duplicate
        }
      } catch(e){}
      return origIAH.call(this, position, html);
    };

    // 4) Mark active tab by URL path
    if (nav) {
